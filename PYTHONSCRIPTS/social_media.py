...
def get_media_by_year(in_year,blogs):
	...
	for blog in blogs:
		...
		if (year==str(in_year)):
			media = blog["shares"].keys()
			for medium in media:
				if (medium in socialmedia) and (blog["shares"][medium]!='error'):
					socialmedia[medium] += int(blog["shares"][medium])
		else:
			continue
...
if __name__ == '__main__':
	...
	all_years = []
	for year in years:
		socialmedia = get_media_by_year(year,blogs)
		print year, socialmedia
		all_years.append(socialmedia)
	...
	plt.savefig("socialmedia_by_year.png")
	plt.show()
	plt.close(fig)
